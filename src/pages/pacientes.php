    ...
    <div class="main-wrapper">
            ...
            <div class="content">
                <div class="row">
                    ...
                    <div class="col-sm-8 col-9 text-right m-b-20">
                        <a href="javascript:void(0);" onclick="redirectMenu('cadastrarPaciente')" class="btn btn btn-primary btn-rounded float-right"><i class="fa fa-plus"></i> Cadastrar Paciente</a>
                    </div>
                </div>
        ...
        ativarMenu('menu-pacientes');
          ...
          table = $('#tabelaPacientes').DataTable({
            ...
            $.ajax({
                url: '/src/ajax/buscarPacientes.php',
                type: 'POST',
                success: function(response) {
                    var response = JSON.parse(response);

                    dados = response.pacientes;
                    ...
                    dados.forEach(function(item) {
                        table.row.add([
                            item.nome ? `<a href="../index.php?page=prontuario&id=${item.id}">${item.nome}</a>` : '',
                            item.telefone || '', // Se "telefone" estiver vazio ou null, insere 'Não Informado'
                            item.cpf || ''       // Se "cpf" estiver vazio ou null, insere 'Não Informado'
                        ]);
